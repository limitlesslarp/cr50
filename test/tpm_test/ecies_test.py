...
_ECIES_COMPAT_INPUTS = (
    (
        ...
        a2b("d61262f22e8c70414777cbc060d1e387"),
        a2b(
            "040c23b1abb7f7e3d2da6ffd70ce9e6f5bf90467c0e1f2e708483d2e61220f0a"
    ...
    (
        ...
        a2b("b3a89ed5a7fb6685a67db54c62e663e7"),
        a2b(
            "04b9d46d1f333baf6896ce7b64d344092671795438b1dc35a21b0d13b004f28a1c"
...
def _encrypt_cmd(auth, in_data, init_vec, pubx, puby, salt, info):
    return (
        _ECIES_OPCODES["ENCRYPT"].to_bytes(1, "big")
        + len(auth + in_data).to_bytes(2, "big")
        + auth
        + in_data
        + len(auth).to_bytes(2, "big")
        + init_vec
        + len(pubx).to_bytes(2, "big")
        + pubx
        + len(puby).to_bytes(2, "big")
        + puby
        + len(salt).to_bytes(2, "big")
        + salt
        + len(info).to_bytes(2, "big")
        + info
    )
...
def _decrypt_cmd(auth, in_data, init_vec, d, salt, info):
    return (
        _ECIES_OPCODES["DECRYPT"].to_bytes(1, "big")
        + len(in_data).to_bytes(2, "big")
        + in_data
        + len(auth).to_bytes(2, "big")
        + init_vec
        + len(d).to_bytes(2, "big")
        + d
        + bytes([0, 0])
        + len(salt).to_bytes(2, "big")
        + salt
    ...
    for data in _ECIES_INPUTS:
        ...
        if not encrypted:
            raise subcmd.TpmTestError(
                "%s error:%s" % (test_name, "null encrypted")
            )
