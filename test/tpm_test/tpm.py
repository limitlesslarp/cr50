...
def tpm2_clear(tpm, handle):
    ...
    clear = [
        ...
        0x00,
        0x01,
        0x26,  # command: TPM_CC_Clear
        # ======
        0x00,
        0x00,
        0x00,
        0x00,  # TPMI_RH_CLEAR handle
        0x00,
        0x00,
        0x00,
        0x09,  # authorizationSize : UINT32
        0x40,
        0x00,
        0x00,
        0x09,  # sessionHandle : empty password
        0x00,
        0x00,
        0x00,
        0x00,
        0x00,  # nonce, sessionAttributes, hmac
    ]
...
def tpm2_nv_read(tpm, index, size, offset):
    ...
    nv_read = [
        ...
        0x3F,
        0xFF,
        0x02,  # authHandle : TPMI_RH_NV_AUTH
        0x01,
        0x3F,
        0xFF,
        0x02,  # nvIndex    : TPMI_RH_NV_INDEX
        0x00,
        0x00,
        0x00,
        0x09,  # authorizationSize : UINT32
    ...
    if len(response) <= 16 or response.count(0) > 100:
        raise subcmd.TpmTestError(
            "Unexpected G2F response: " + utils.hex_dump(response)
        )
    return response
...
def tpm2_create_ek(tpm):
    ...
    create_primary_ek = [
        ...
        0x00,
        0x00,
        0xA3,  # size = 0xd7 = 215 - 52 = 163 = a3
        0x00,
        0x00,
        0x01,
        0x31,  # code = TPM_CC_CreatePrimary
        # ------
        0x40,
        0x00,
        0x00,
        0x0B,  # primary_handle =  TPM_RH_ENDORSEMENT
        # ~~~~~~
        0x00,
        0x00,
        0x00,
        ...
        0x00,  # password: TPM2B
        0x00,
        0x04,  # sensitive.size = 4
        0x00,
        0x00,  # sensitive.auth.size = 0
        0x00,
        0x00,  # sensitive.data.size = 0
        0x00,
        0x7A,  # public.size = 122
        0x00,
        0x23,  # public.type = TPM_ALG_ECC
        0x00,
        0x0B,  # public.name_alg = SHA256
        0x00,
        0x03,
        0x00,
        0xB2,  # attributes
        0x00,
        0x20,  # public.auth_policy.size = 32 (below)
        0x83,
        0x71,
        0x97,
        0x67,
        ...
        0x00,
        0x06,  # public.sym.alg = TPM_ALG_AES
        0x00,
        0x80,  # public.sym.key_bits = 128
        0x00,
        0x43,  # public.sym.mode = TPM_ALG_CFB
        0x00,
        0x10,  #
        0x00,
        0x03,
        ...
        0x10,
        0x00,
        0x20,  # public_area->unique.ecc.x.b.size
        0x00,
        0x00,
        0x00,
        0x00,
        ...
        0x00,
        0x00,
        0x20,  # public_area->unique.ecc.y.b.size
        0x00,
        0x00,
        0x00,
        0x00,
        ...
        0x00,
        0x00,
        0x00,  # outside_info.size = 0
        0x00,
        0x00,
        0x00,
        0x00,  # pcr_select.size = 0
    ]
...
def tpm2_flush_context(tpm, handle: bytes):
    ...
    flush_context = [
        ...
        0x00,
        0x01,
        0x65,  # command: TPM_CC_FlushContext
        0xFF,
        0xFF,
        0xFF,
        0xFF,  # handle: filled below
    ]
