...
def _drbg_init_cmd(drbg_op, entropy, nonce, perso):
    return (
        drbg_op.to_bytes(1, "big")
        + len(entropy).to_bytes(2, "big")
        + entropy
        + len(nonce).to_bytes(2, "big")
        + nonce
        + len(perso).to_bytes(2, "big")
        + perso
    )
...
def _drbg_gen_cmd(inp, outlen):
    return (
        DRBG_GENERATE.to_bytes(1, "big")
        + len(inp).to_bytes(2, "big")
        + inp
...
def drbg_init(tpm, drbg_params):
    ...
    if response != EMPTY_DRBG_RESPONSE:
        raise subcmd.TpmTestError(
            ...
            "DRBG_INIT: %s" % (utils.hex_dump(response))
        )
...
def drbg_reseed(tpm, drbg_params):
    ...
    if response != EMPTY_DRBG_RESPONSE:
        raise subcmd.TpmTestError(
            "Unexpected response to "
            "DRBG_RESEED: %s" % (utils.hex_dump(response))
        )
    ...
    if check_result:
        ...
        if expected and a2b(expected) != result:
            raise subcmd.TpmTestError(
                "error:\nexpected %s"
                "\nreceived %s"
                % (utils.hex_dump(a2b(expected)), result_hexdump)
            )
