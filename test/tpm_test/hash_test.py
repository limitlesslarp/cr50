...
EMPTY_RESPONSE = bytes(
    [0x80, 0x01, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01]
)
TEST_INPUTS = (
    ...
    (
        ...
        0,
        "hmac_key1",
        "some text, this time for sw hmac",
    ),
    ...
    (
        ...
        0,
        "hmac_key5",
        "some text, this time for hw hmac",
    ),
    (
        CMD_HMAC_SW,
        ALG_SHA256,
        0,
        "very long hmac_key 456456789012345",
        "   text",
    ),
    (
        CMD_HMAC_HW,
        ALG_SHA256,
        0,
        "very long hmac_key 123456789012345",
        "   text",
    ),
    (
        CMD_HMAC_SW,
    ...
    (
        ...
        1,
        "",
        "some more text, this time for sha256",
    ),
...
def hash_test(tpm):
    ...
    for test in TEST_INPUTS:
        ...
        cmd = struct.pack(">BBBH", hash_cmd, hash_alg, handle, len(text)) + text
        # for HMAC add key
        ...
        if result != hash_context.digest():
            raise subcmd.TpmTestError(
                "%s error:%s%s"
                % (test_name, utils.hex_dump(digest), utils.hex_dump(result))
            )
