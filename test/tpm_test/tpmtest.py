...
class TPM:
    ...
    def validate(self, data_blob, response_mode=False, check_cmd=True):
        ...
        if size != len(data_blob):
            raise subcmd.TpmTestError(
                prefix
                + "size mismatch: header %d, actual %d" % (size, len(data_blob))
            )
        if size > 4096:
    ...
    def unwrap_ext_response(self, expected_subcmd, response):
        ...
        tag, size, cmd, sub = struct.unpack(
            self.HEADER_FMT, response[:header_size]
        )
        if tag != 0x8001:
...
def check_for_run(requested_test):
    ...
    if not found_valid_test:
        raise ValueError(
            "%r is not a valid test. Use one of %r"
...
def usage():
    ...
    print(
        "Syntax: tpmtest.py [-d] | [-t source [-o file] [-s bits] ]| -h | "
        "                           -T test_name | -p | -b ]\n"
...
def main():
    ...
    try:
        ...
        if trng_only:
            trng_test.trng_test(
                tpm_object, trng_output, trng_mode, trng_sample_bits
            )
            sys.exit(0)
        if run_test(requested_test, "u2f"):
            u2f_test.u2f_test(tpm_object)
        if run_test(requested_test, "crypto"):
            crypto_test.crypto_tests(
                tpm_object, os.path.join(ROOT_DIR, "crypto_test.xml")
            )
        if run_test(requested_test, "drbg"):
            drbg_test.drbg_test(tpm_object, request, expected, lab_output)
