...
_SIGN_MODE = {"NONE": 0x00, "ECDSA": 0x18}

# TPM2 ALG codes.
_HASH = {"NONE": 0x10, "SHA1": 0x04, "SHA256": 0x0B}

_HASH_FUNC = {"NIST-P256": hashlib.sha256}
...
def _verify_cmd(curve_id, hash_func, sign_mode, msg, sig):
    ...
    return (
        struct.pack(
            ">BBBB", _ECC_OPCODES["VERIFY"], curve_id, sign_mode, _HASH["NONE"]
        )
        + sig
        + len(digest).to_bytes(2, "big")
        + digest
    )
...
def _verify_any_cmd(curve_id, hash_func, sign_mode, msg, sig, q_x, q_y):
    ...
    return (
        ...
        + len(digest).to_bytes(2, "big")
        + digest
        + len(q_x).to_bytes(2, "big")
        + q_x
        + len(q_y).to_bytes(2, "big")
        + q_y
    )
...
def _test_point_cmd(curve_id, q_x, q_y):
    return (
        struct.pack(">BB", _ECC_OPCODES["TEST_POINT"], curve_id)
        + len(q_x).to_bytes(2, "big")
        + q_x
        + len(q_y).to_bytes(2, "big")
        + q_y
...
def _keyderive_cmd(curve_id, seed):
    return (
        struct.pack(">BBH", _ECC_OPCODES["KEYDERIVE"], curve_id, len(seed))
        + seed
    )


_SIGN_INPUTS = (("NIST-P256", "ECDSA"),)


_KEYGEN_INPUTS = (("NIST-P256",),)
...
def _sign_test(tpm):
    ...
    for data in _SIGN_INPUTS:
        ...
        cmd = _sign_cmd(
            _ECC_CURVES[curve_id],
            _HASH_FUNC[curve_id],
            _SIGN_MODE[sign_mode],
            msg,
        )
...
def _keygen_test(tpm):
    for data in _KEYGEN_INPUTS:
        ...
        counter = 1
        while counter < 100:
            wrapped_response = tpm.command(
                tpm.wrap_ext_command(subcmd.ECC, cmd)
            )
