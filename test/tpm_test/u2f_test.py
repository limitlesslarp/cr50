...
def u2f_generate(tpm, origin, user, flag, auth):
    ...
    wrapped_response = tpm.command(
        tpm.wrap_ext_command(subcmd.U2F_GENERATE, cmd)
    )
    ...
    if fail == False:
        ...
        if size != 12:
            raise subcmd.TpmTestError(
                "Unexpected response: " + utils.hex_dump(response)
            )
...
def u2f_attest(
    tpm, origin, user, challenge, kh, public_key, corp_format=False, fail=False
):
    origin = origin[:32].ljust(32, b"\0")
    user = user[:32].ljust(32, b"\0")
    if not corp_format:
...
def u2f_test(tpm):
    ...
    sig_attest = u2f_attest(
        tpm, origin, user, auth, khv0, public_key0, corp_format=True
    )
    if tpm.debug_enabled():
        print("sig attest = ", utils.hex_dump(sig_attest), len(sig_attest))
